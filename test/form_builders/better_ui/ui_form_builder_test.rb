...
module BetterUi
  class UiFormBuilderTest < ActiveSupport::TestCase
    ...
    # ui_text_input tests
    test "ui_text_input renders TextInputComponent" do
      @user.name = "John Doe"
    ...
    test "ui_text_input uses humanized attribute name as default label" do
      output = @builder.ui_text_input(:name)

      assert_match(/Name/, output)
    end

    test "ui_text_input accepts custom label" do
      output = @builder.ui_text_input(:name, label: "Full Name")

      assert_match(/Full Name/, output)
    end

    test "ui_text_input accepts hint text" do
      output = @builder.ui_text_input(:email, hint: "We'll never share your email")

      # HTML entities are encoded, so we need to check for the decoded version
      assert_match(/We&#39;ll never share your email/, output)
    end

    test "ui_text_input accepts placeholder" do
      output = @builder.ui_text_input(:email, placeholder: "you@example.com")

      assert_match(/placeholder="you@example.com"/, output)
    end

    test "ui_text_input accepts size option" do
      output = @builder.ui_text_input(:name, size: :lg)

      assert_match(/text-lg/, output)
    end

    test "ui_text_input shows errors from model" do
      @user.valid? # trigger validations
      output = @builder.ui_text_input(:email)

      # HTML entities are encoded, so we need to check for the encoded version
      assert_match(/Email can&#39;t be blank/, output)
    end

    test "ui_text_input marks required fields based on validations" do
      output = @builder.ui_text_input(:name)

      assert_match(/required/, output)
    end

    test "ui_text_input accepts block for icon slots" do
      output = @builder.ui_text_input(:email) do |component|
    ...
    test "ui_text_input accepts disabled option" do
      output = @builder.ui_text_input(:name, disabled: true)

      assert_match(/disabled/, output)
    end

    test "ui_text_input accepts readonly option" do
      output = @builder.ui_text_input(:name, readonly: true)

      assert_match(/readonly/, output)
    end

    # ui_number_input tests
    test "ui_number_input renders NumberInputComponent" do
      @user.age = 25
      output = @builder.ui_number_input(:age)
    ...
    test "ui_number_input accepts min option" do
      output = @builder.ui_number_input(:age, min: 0)

      assert_match(/min="0"/, output)
    end

    test "ui_number_input accepts max option" do
      output = @builder.ui_number_input(:age, max: 120)

      assert_match(/max="120"/, output)
    end

    test "ui_number_input accepts step option" do
      output = @builder.ui_number_input(:price, step: 0.01)

      assert_match(/step="0.01"/, output)
    end

    test "ui_number_input accepts all numeric options" do
      output = @builder.ui_number_input(:price, min: 0, max: 1000, step: 0.01)
    ...
    test "ui_number_input accepts show_spinner option" do
      output = @builder.ui_number_input(:age, show_spinner: false)

      assert_match(/hide-number-spinner/, output)
    end

    test "ui_number_input shows spinner by default" do
      output = @builder.ui_number_input(:age)

      refute_match(/hide-number-spinner/, output)
    end

    test "ui_number_input accepts block for icon slots" do
      output = @builder.ui_number_input(:price) do |component|
    ...
    test "ui_number_input shows errors from model" do
      @user.age = -5
      @user.valid?
      output = @builder.ui_number_input(:age)
    ...
    # ui_password_input tests
    test "ui_password_input renders PasswordInputComponent" do
    ...
    test "ui_password_input accepts hint text" do
      output = @builder.ui_password_input(:password, hint: "Minimum 8 characters")

      assert_match(/Minimum 8 characters/, output)
    end

    test "ui_password_input accepts block for prefix icon" do
      output = @builder.ui_password_input(:password) do |component|
    ...
    test "ui_password_input can be marked as required" do
      # Explicitly mark password as required
      output = @builder.ui_password_input(:password, required: true)

      assert_match(/required/, output)
    end

    # ui_textarea tests
    test "ui_textarea renders TextareaComponent" do
      @user.bio = "Sample bio"
      output = @builder.ui_textarea(:bio)
    ...
    test "ui_textarea has default rows" do
      output = @builder.ui_textarea(:bio)

      assert_match(/rows="4"/, output)
    end

    test "ui_textarea accepts custom rows" do
      output = @builder.ui_textarea(:bio, rows: 8)

      assert_match(/rows="8"/, output)
    end

    test "ui_textarea accepts cols option" do
      output = @builder.ui_textarea(:bio, cols: 80)

      assert_match(/cols="80"/, output)
    end

    test "ui_textarea accepts maxlength option" do
      output = @builder.ui_textarea(:bio, maxlength: 500)

      assert_match(/maxlength="500"/, output)
    end

    test "ui_textarea accepts resize option" do
      output = @builder.ui_textarea(:bio, resize: :none)

      assert_match(/resize-none/, output)
    end

    test "ui_textarea has vertical resize by default" do
      output = @builder.ui_textarea(:bio)

      assert_match(/resize-y/, output)
    end

    test "ui_textarea accepts hint text" do
      output = @builder.ui_textarea(:bio, hint: "Tell us about yourself")

      assert_match(/Tell us about yourself/, output)
    end

    test "ui_textarea accepts block for icon slots" do
      output = @builder.ui_textarea(:bio) do |component|
    ...
    test "handles nested attributes in field names" do
      ...
      output = nested_builder.ui_text_input(:name)
    ...
    test "populates values from model object" do
      ...
      name_output = @builder.ui_text_input(:name)
      email_output = @builder.ui_text_input(:email)
      age_output = @builder.ui_number_input(:age)
    ...
    test "accepts custom container classes" do
      output = @builder.ui_text_input(:name, container_classes: "custom-wrapper")

      assert_match(/custom-wrapper/, output)
    end

    test "accepts custom label classes" do
      output = @builder.ui_text_input(:name, label_classes: "custom-label")

      assert_match(/custom-label/, output)
    end

    test "accepts custom input classes" do
      output = @builder.ui_text_input(:name, input_classes: "custom-input")
    ...
    test "renders complete form with all input types" do
      ...
      name_output = @builder.ui_text_input(:name, hint: "Your full name")
      email_output = @builder.ui_text_input(:email)
      age_output = @builder.ui_number_input(:age, min: 0, max: 120)
      password_output = @builder.ui_password_input(:password, hint: "Min 8 characters")
      bio_output = @builder.ui_textarea(:bio, rows: 6)
    ...
    test "handles missing attribute gracefully" do
      output = @builder.ui_text_input(:nonexistent_field)
